...
script <- script %>% select(Speaker, Listener)
...
script <- script %>% filter(Listener != "")
script <- script %>% filter(Speaker != "")

# there are 54 different Speakers
length(unique(script$Speaker))
unique(script$Speaker)

# there are 50 different Listeners
length(unique(script$Listener))
unique(script$Listener)
...
conversations <- script %>% group_by(Speaker, Listener) %>% summarise(counts = n())
...
plot(g, 
     ...
     edge.color = 'orange',
     edge.arrow.size = .008,
     layout = layout_components(my_graph),
     margin = -0.45)

#using tkplot for better visualisation
tkplot(g, 
     vertex.label.color = "black",
     vertex.label.cex = 1,
     vertex.size = 8,
